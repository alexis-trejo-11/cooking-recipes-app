...
class RecipeCollections:
    ...
    def reorder_steps(self, new_order: List[int]) -> "RecipeCollections":
        """Reordenar pasos."""
        if len(new_order) != len(self._steps):
            raise ValueError("New order must contain all steps")

        if set(new_order) != set(range(1, len(self._steps) + 1)):
            raise ValueError(
                "New order must contain sequential numbers starting from 1"
            )

        # Crear mapping de old index to new number
        step_mapping = {i + 1: self._steps[i] for i in range(len(self._steps))}

        # Recrear steps con nuevos números
        reordered_steps = []
        for new_number in new_order:
            old_step = step_mapping[new_number]
            reordered_steps.append(
                Step(
                    number=len(reordered_steps) + 1,
                    description=old_step.description,
                    duration_minutes=old_step.duration_minutes,
                    technique=old_step.technique,
                    temperature=old_step.temperature,
                    ingredients_used=old_step.ingredients_used,
                )
            )
    ...
    def remove_tag(self, tag: Tag) -> "RecipeCollections":
        """Remover etiqueta."""
        new_tags = self._tags - {tag}
        return RecipeCollections(
            ingredients=self._ingredients,
